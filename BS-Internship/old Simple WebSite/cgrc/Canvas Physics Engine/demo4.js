...
function Particle(){
    ...
    this.bindedShape = new Path.Circle(this.position, 10);
...
window.onload = function() {
    ...
    pfrn0.particleForceGenerator = new ParticleSpring(bungeeEnd, 15, 300);
    var path = new Path();
    path.add(bungeeEnd.position);
    path.add(particle.position);
    particle.path = path;
    path.strokeColor = 'black';
    pfr.registrations.push(pfrn0);
    ...
    for(var i=0;i<12;i++){
        ...
        particle2.position.x = 150 + i*100;
        particle2.position.y = 400;
    ...
    world.registry = pfr;
    var gameOver = false;
    var particleMove = null;
    tool.onMouseDown = function(event){
        
        var particle = world.firstParticle;
        while(particle){
            var hit = particle.particle.bindedShape.hitTest(event.point);
            if(hit){
                particleMove = particle;
            }
...
//        gameOver = !gameOver;
    ...
    tool.onMouseDrag = function(event){
        if(particleMove){
            particleMove.particle.position.y = event.point.y;
        }
    }
