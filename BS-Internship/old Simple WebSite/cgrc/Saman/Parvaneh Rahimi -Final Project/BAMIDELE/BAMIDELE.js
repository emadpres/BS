...
window.onload = function() {
    ...
    $('#myCanvas_div_no_cursor').css('cursor', 'none');
    ...
    stops[0] = [['#FFCC99', 0], ['#CC6633', 0.75], ['#CC6633', 1]];
    ...
    var gradient = new Gradient(stops[0], 'radial');
    ...
    view.onFrame = function(event){
        ...
        if(level==1){
            if(music.currentTime>90 && music.currentTime<208){
                level=2;
    ...
    function placeTambourines(){
        ...
        for (var d = 1; d < 3; d++) {
            if(d==1){
                point = new Point(place*d,y+95);
            }else{
                point = new Point(place*d+320,y+95);
            }

            var  placedTambourine = tambourineSymbol.place(point);
            tambourines[d-1] = placedTambourine;
        }
    }
    function random(){
        ...
        if(hitResult){
            ...
            if(mode=="defence"){
                ...
                for(var i=pathHitIndex;i<size;i++){
                    ...
                    if(temp<100){
                        attackedEvolutionShape.add(new Point(segments[i].point.x + attackVector2.x,segments[i].point.y + attackVector2.y));
                    }else{
                        attackedEvolutionShape.add(new Point(segments[i].point.x - attackVector2.x,segments[i].point.y - attackVector2.y));
                    }
                }
                for(var j=pathHitIndex-1;j>=0;j--){
                    ...
                    if(temp2<100){
                        attackedEvolutionShape.insert(0,new Point(segments[j].point.x + attackVector2.x,segments[j].point.y + attackVector2.y));
                    }else{
                        attackedEvolutionShape.insert(0,new Point(segments[j].point.x - attackVector2.x,segments[j].point.y - attackVector2.y)); 
                    }
                }
    ...
    function pathUpdate(){
        ...
        if(mode=="normal"||mode=="attackRecovery"||mode=="endofGame"||mode=="endofGameJam"){
