...
int mainprocess(int m[][22],int a,int b,int start)
{
    ...
    {    
         ...
         {       
            ...
            if (getpixel(x,y)==GREEN){return true;}
            else if (getpixel(x,y)==RED)return false;
         }
     }
...
bool updatemap(int size,int x,int y,bool left, bool right,bool *first)
{
    ...
    {   
        ...
        else if(map[y][x]==1)
        {
             ...
             if (counter==8)
             {map[y-1][x-1]=map[y][x-1]=map[y+1][x-1]=map[y-1][x]=map[y+1][x]=map[y-1][x+1]=map[y][x+1]=map[y+1][x+1]=map[y][x]=3;}
             else 
             {
...
bool checkwin(int size)
{
     ...
     if (counter==(size*size))return true;
     else return false;
...
int secondpage()
{
    ...
    cout<<"by  right click ont the box you can flag and unflag it"<<endl;
