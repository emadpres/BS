...
void ProcessDoc(int fileNumber)
{
		...
		while ( temp != NULL)
		{
			...
			if (stop.find(c) == stop.end())
			{
				...
				if (invertedIndex.find(c) == invertedIndex.end())
				{
					...
					invertedIndex[string(c)].second = new CDocsFreq(fileNumber);
				}
				else
				{
					...
					{
						invertedIndex[c].first++;
						invertedIndex[c].second = new CDocsFreq(fileNumber, invertedIndex[c].second);
					}
				}
			}
...
int main()
{
	...
	while ( (ent = readdir(dir)) )
	{
		...
		docs.push_back(strcpy(new char[500],ent->d_name));
