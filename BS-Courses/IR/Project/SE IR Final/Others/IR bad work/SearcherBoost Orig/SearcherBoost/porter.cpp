...
int CPorter::k;
int CPorter::k0;
int CPorter::stem(char * p, int i, int j)
{
...
int CPorter::vowelinstem()
{
	int i; for (i = k0; i <= j; i++) if (! cons(i)) return TRUE;
	return FALSE;
}

int CPorter::doublec(int j)
{
...
void CPorter::r(char * s)
{
	if (m() > 0) setto(s);
}

void CPorter::step1ab()
{
	if (b[k] == 's')
	{
