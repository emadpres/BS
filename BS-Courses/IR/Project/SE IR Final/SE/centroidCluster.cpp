...
CCentroidCluster::CCentroidCluster(CSearcher *pParent)
{
	mParent = pParent;
	N = mParent->docs.size();
...
void CCentroidCluster::Cluster(int k)
{
	int n = N, x = -1, y = -1;
	long double max = 0;
	for (int i = 0; i < N; i++)
...
void CCentroidCluster::Merge(int pCluster1, int pCluster2)
{
...
long double CCentroidCluster::Similarity(vector<pair<string, long double> > &pDoc1, vector<pair<string, long double> > &pDoc2)
{
	long double cos = 0;
