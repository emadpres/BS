...
//	OthelloAbstractBoard abstractBoard = board->getState();
	...
	if (position->board[0][0]==WHITE)
		position->whiteHasCornerUpLeft = true;
	if (position->board[0][GAMESIZE-1]==WHITE)
		position->whiteHasCornerUpRight = true;
	if (position->board[GAMESIZE-1][0]==WHITE)
		position->whiteHasCornerDownLeft = true;
	if (position->board[GAMESIZE-1][GAMESIZE-1]==WHITE)
		position->whiteHasCornerDownRight = true;

	if (position->board[0][0]==BLACK)
		position->blackHasCornerUpLeft = true;
	if (position->board[0][GAMESIZE-1]==BLACK)
		position->blackHasCornerUpRight = true;
	if (position->board[GAMESIZE-1][0]==BLACK)
		position->blackHasCornerDownLeft = true;
	if (position->board[GAMESIZE-1][GAMESIZE-1]==BLACK)
		position->blackHasCornerDownRight = true;
